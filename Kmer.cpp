...
static const uint8_t base_cmp[256] = {
...
bool Kmer::operator<(const Kmer& o) const {
  ...
  //return memcmp(bytes,o.bytes,MAX_K/4) < 0;
    ...
    for (size_t i = 0; i < k_bytes-1; ++i) {
      ...
      if (base_cmp[bytes[i]] > base_cmp[o.bytes[i]]) {
        return false;
      } else if (base_cmp[bytes[i]] < base_cmp[o.bytes[i]]) {
	return true;
      }
    }
    ...
    if (base_cmp[bytes[k_bytes-1] & k_modmask] >= base_cmp[o.bytes[k_bytes-1] & k_modmask]) {
      return false;
    }
    return true;
}
