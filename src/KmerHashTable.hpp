...
// ---- iterator ----
    ...
    class iterator_ : public std::iterator<std::forward_iterator_tag, packed_tiny_vector> {
        ...
        public:
            ...
            MHT_val1_ref_t operator*() const { return ht->table_values_1[h]; }
            MHT_val1_ptr_t operator->() const { return &(ht->table_values_1[h]); }

            const Minimizer& getKey() const { return ht->table_keys[h]; }

            MHT_val1_ref_t getVal1() const { return ht->table_values_1[h]; }
            MHT_val2_ref_t getVal2() const { return ht->table_values_2[h]; }
