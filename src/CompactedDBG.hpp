...
* Prefix for the name of the file to which the graph must be written (not used by CompactedDBG<T>::build).
* (not used by CompactedDBG<T>::build). Mandatory parameter.
