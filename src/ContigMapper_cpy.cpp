...
bool ContigMapper::addContig(Kmer km, const string& read, size_t pos, const string& seq) {
    ...
    if (!found) {
        ...
        // proper new contig
        if (s.size()-k+1 < limit && !selfLoop) sContigs.insert(make_pair(head, CompressedCoverage(s.size()-k+1))); // create a short contig
        else lContigs.insert(make_pair(head, new Contig(c)));

        // insert shortcuts every stride k-mers
        for (size_t i = stride; i < len-k; i += stride) shortcuts.insert(make_pair(Kmer(c+i),make_pair(head,i)));
...
ContigMap ContigMapper::findContig(Kmer km, const string& s, size_t pos) const {
    ...
    ContigMap cc = this->find(end); //look for a contig from this k-mer

    if (!cc.isEmpty && !cc.isShort) { //if it is found and its not a short contig

        const CompressedSequence& seq = lContigs.find(cc.head)->second->seq; // ok, fetch the sequence
        size_t km_dist = cc.dist; //Distance of the k-mer from start of contig
        size_t jlen = 0;
...
ContigMap ContigMapper::find(Kmer km) const {
    ...
    else if ((sc_it = shortcuts.find(km)) != shortcuts.end()) {
    ...
    for (hmap_long_contig_t::iterator it = lContigs.begin(); it != lContigs.end(); ++it) {
        ...
        if (c->length() < k) { // check the strict inequality here
            ...
            if (seq.size() > k) {
                ...
                for (size_t i = stride; i < seq.size() - k; i+= stride) shortcuts.insert(make_pair(seq.getKmer(i), make_pair(it->first, i)));
...
size_t ContigMapper::removeIsolatedContigs() {
    ...
    for (auto& kv : lContigs) {
        ...
        for (size_t i = 0; i < 4; i++) {
            ...
            if (checkEndKmer(fw, dummy)) fw_count++;
            Kmer bw = head.backwardBase(alpha[i]);
            if (checkEndKmer(bw, dummy)) bw_count++;
        }

        if (fw_count == 0 && bw_count == 0) rems.push_back(kv.first);
    }
...
size_t ContigMapper::joinAllContigs() {
    ...
    for (vector<Join_t>::iterator it = joins.begin(); it != joins.end(); ++it) {
        ...
        if (!cHead.isEmpty && !cTail.isEmpty) {
            ...
            if (head == headContig->seq.getKmer(headContig->seq.size()-k)) headDir = true;
            else if (head.twin() == headContig->seq.getKmer(0)) headDir = false;
            else continue; // can't join up

            if (tail == tailContig->seq.getKmer(0)) tailDir = true;
            else if (tail.twin() == tailContig->seq.getKmer(tailContig->seq.size()-k)) tailDir = false;
            else continue; // can't join up
...
bool ContigMapper::checkEndKmer(Kmer b, bool& dir) {
    ...
    else return false; // doesn't match, can this ever happen?
}
    ...
    */
    ...
    for (hmap_short_contig_t::iterator it = sContigs.begin(); it != sContigs.end(); ) {
        ...
        if (! it->second.isFull()) {
            ...
            if (selfLoop) cout << "Splitting a self-loop, not implemented" << endl;
...
void ContigMapper::removeShortcuts(const string& s) {
    ...
    for (size_t i = stride; i < s.size()-k+1; i += stride) shortcuts.erase(Kmer(c+i));
...
size_t ContigMapper::writeGFA(int count1, string graphfilename, bool debug = false) {
    ...
    else graph.rdbuf(cout.rdbuf()); // copy to cout
