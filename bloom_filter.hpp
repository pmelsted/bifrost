...
public:
  ...
  bloom_filter(const std::size_t& predicted_element_count,
	       const std::size_t& bits_per_element,
	       const std::size_t& random_seed)
    : bit_table_(0),
      predicted_element_count_(predicted_element_count),
      inserted_element_count_(0),
      random_seed_((random_seed) ? random_seed : 0xDEADBEEF)
  {
    table_size_ = bits_per_element * predicted_element_count_;
    bit_table_ = new cell_type[table_size_ / bits_per_char];
    salt_count_ = (std::size_t) (std::log(2.0) * bits_per_element);
    generate_unique_salt();
    std::fill_n(bit_table_ ,(table_size_ / bits_per_char), 0x00);
    desired_false_positive_probability_ = std::pow(1.0-std::exp(-1.0 * salt_.size() * predicted_element_count_ / size()), 1.0 * salt_.size());
  }
