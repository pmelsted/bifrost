...
* **04-28-2022**
	...
	* `CompactedDBG::simplify()` and `ColoredCDBG::simplify()` now return true even if no simplification was performed ("null-simplification" in case all input parameters are set to false). The goal is to have this function return false only if the graph is invalid or in case of unexpected behavior. 
* **08-29-2018**
